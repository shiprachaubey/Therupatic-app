...
export default function GeneralPhysicianScreen() {
  const navigation = useNavigation();
  const [pdfUrl, setPdfUrl] = useState(null);
  const LOCAL_PDF_PATH = `${RNFetchBlob.fs.dirs.DocumentDir}/gp_ortho.pdf`; // Local file path
  ...
  useEffect(() => {
    const loadPdf = async () => {
      try {
        ...
        const remotePdfPath = generalCategory?.pdffiles?.[0]?.pdf_file;
        if (!remotePdfPath) {
          console.warn('No PDF found for GP and Ortho');
          return;
        }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      >
        ...
        <Text style={styles.cardLabel}>
          {pdfUrl ? 'GENERAL PHYSICIAN & ORTHOPEDIC FOLDER' : 'Loading PDF...'}
        </Text>
      </TouchableOpacity>
